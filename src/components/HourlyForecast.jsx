      ...
      <div className="hide-scrollbar flex gap-2 overflow-x-auto pb-1 sm:gap-3">
        {data?.hourly?.map((item, index) => (
          ...
          >
            ...
            <p className={`text-sm font-semibold sm:text-lg ${isDark ? 'text-white' : 'text-slate-800'}`}>{item.temp}°</p>
            <p className={`mt-1 text-[11px] leading-tight sm:text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{item.rain}% rain</p>
          </motion.div>
        ))}
      </div>
    </div>
