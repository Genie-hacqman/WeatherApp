  ...
  return (
    <div className={`rounded-[28px] border p-4 shadow-[0_16px_48px_rgba(0,0,0,0.24)] backdrop-blur-xl sm:p-5 ${isDark ? 'border-white/10 bg-slate-950/60' : 'border-slate-200/80 bg-white/85 shadow-[0_16px_48px_rgba(15,23,42,0.12)]'}`}>
      ...
      {selectedItem && (
        ...
        >
          <p className={`text-xs uppercase tracking-[0.2em] ${isDark ? 'text-orange-300' : 'text-orange-600'}`}>Selected day</p>
          <div className="mt-2 flex items-center justify-between">
            ...
            <div className={`text-right text-sm ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
              <p>{selectedItem.high}° / {selectedItem.low}°</p>
              <p>Rain chance {selectedItem.rain}%</p>
            </div>
          </div>
        </motion.div>
      )}
      ...
      <div className="space-y-2 sm:space-y-3">
        {data?.daily?.map((item, index) => (
          <motion.button
            ...
            className={`flex w-full flex-col gap-2 rounded-[20px] border px-3 py-2 text-left transition sm:flex-row sm:items-center sm:justify-between sm:px-4 sm:py-3 ${isDark ? 'border-white/10 bg-white/10 hover:bg-white/20' : 'border-slate-300/60 bg-white/80 hover:bg-slate-50'} ${selectedItem?.day === item.day ? (isDark ? 'ring-1 ring-orange-400/50' : 'ring-1 ring-orange-300') : ''}`}
          >
